import React from 'react';
import { Link } from 'react-router-dom';

const coursesData = [
  {
    id: 1,
    title: '.NET Full Stack',
    image: 'data:image/jpeg;base64,', // .NET logo
    duration: '4 Months',
    cost: '₹24,000',
  },
  {
    id: 2,
    title: 'Java Full Stack',
    image: 'https://cdn-icons-png.flaticon.com/512/226/226777.png', // Java logo
    duration: '4–6 Months',
    cost: '₹30,000',
  },
  {
    id: 3,
    title: 'MEAN Full Stack',
    image: 'data:image/jpeg;base64,', // MEAN stack logos (Mongo, Express, Angular, Node)
    duration: '4 Months',
    cost: '₹22,000',
  },
  {
    id: 4,
    title: 'MERN Full Stack',
    image: 'data:image/jpeg;base64,', // React logo to represent MERN
    duration: '3 Months',
    cost: '₹22,000',
  },
  {
    id: 5,
    title: 'Python Full Stack',
    image: 'https://cdn-icons-png.flaticon.com/512/5968/5968350.png', // Python logo
    duration: '6 Months',
    cost: '₹28,000',
  },
  {
    id: 6,
    title: 'PHP Full Stack',
    image: 'https://cdn-icons-png.flaticon.com/512/919/919830.png', // PHP logo
    duration: '3 Months',
    cost: '₹18,000',
  },
  {
    id: 7,
    title: 'Data Science & ML',
    image: 'https://cdn-icons-png.flaticon.com/512/4140/4140043.png', // Data Science
    duration: '6 Months',
    cost: '₹60,000',
  },
  {
    id: 8,
    title: 'Cyber Security & Ethical Hacking',
    image: 'https://cdn-icons-png.flaticon.com/512/633/633600.png', // Hacking/security icon
    duration: '12 Months',
    cost: '₹80,000',
  },
  {
    id: 9,
    title: 'Web Designing',
    image: 'https://cdn-icons-png.flaticon.com/512/1006/1006771.png', // Design icon
    duration: '4 Months',
    cost: '₹18,000',
  },
  {
    id: 10,
    title: 'MongoDB',
    image: 'https://cdn.iconscout.com/icon/free/png-256/mongodb-4-1175139.png',
    duration: '2 Months',
    cost: '₹8,000',
  },
  {
    id: 11,
    title: 'React JS',
    image: 'https://upload.wikimedia.org/wikipedia/commons/a/a7/React-icon.svg',
    duration: '2 Months',
    cost: '₹10,000',
  },
  {
    id: 12,
    title: 'Artificial Intelligence',
    image: 'https://cdn-icons-png.flaticon.com/512/4397/4397313.png',
    duration: '2 Months',
    cost: '₹70,000',
  },
  {
    id: 13,
    title: 'C#',
    image: 'data:image/png;base64,',
    duration: '2 Months',
    cost: '₹6,000',
  },
  {
    id: 14,
    title: 'Computer Hardware',
    image: 'https://cdn-icons-png.flaticon.com/512/222/222474.png',
    duration: '2 Months',
    cost: '₹2,000',
  },
  {
    id: 15,
    title: 'JavaScript',
    image: 'https://cdn-icons-png.flaticon.com/512/5968/5968292.png',
    duration: '2 Months',
    cost: '₹8,000',
  },
  {
    id: 16,
    title: 'Node JS',
    image: 'https://nodejs.org/static/images/logo.svg',
    duration: '2 Months',
    cost: '₹8,000',
  },
  {
    id: 17,
    title: 'Python',
    image: 'https://cdn-icons-png.flaticon.com/512/5968/5968350.png',
    duration: '2 Months',
    cost: '₹6,000',
  },
  {
    id: 18,
    title: 'Angular JS',
    image: 'https://angular.io/assets/images/logos/angular/angular.svg',
    duration: '2 Months',
    cost: '₹8,000',
  },
  {
    id: 19,
    title: 'C',
    image: 'https://upload.wikimedia.org/wikipedia/commons/1/19/C_Logo.png',
    duration: '2 Months',
    cost: '₹5,000',
  },
  {
    id: 20,
    title: 'C++',
    image: 'https://upload.wikimedia.org/wikipedia/commons/1/18/ISO_C%2B%2B_Logo.svg',
    duration: '2 Months',
    cost: '₹5,000',
  },
  {
    id: 21,
    title: 'Data Science',
    image: 'https://cdn-icons-png.flaticon.com/512/4140/4140043.png',
    duration: '2 Months',
    cost: '₹20,000',
  },
  {
    id: 22,
    title: 'Ethical Hacking',
    image: 'https://cdn-icons-png.flaticon.com/512/633/633600.png',
    duration: '2 Months',
    cost: '₹70,000',
  },
  {
    id: 23,
    title: 'SQL Server Database',
    image: 'data:image/png;base64,',
    duration: '2 Months',
    cost: '₹6,000',
  },
  {
    id: 24,
    title: 'Java',
    image: 'https://cdn-icons-png.flaticon.com/512/226/226777.png',
    duration: '2 Months',
    cost: '₹12,000',
  },
];

const whatsappNumber = '918287584509';
const sampleMessage = encodeURIComponent(
  "Hi, I'm interested in enrolling. Please share details!"
);

const Courses = () => (
  <div className="min-h-screen bg-black py-12 px-4 text-white">
    <div className="max-w-7xl mx-auto">
      <h2 className="text-3xl font-bold mb-8 text-center">Courses</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 gap-8">
        {coursesData.map((course) => (
          <div
            key={course.id}
            className="bg-gray-900 rounded-lg shadow-lg p-5 flex flex-col items-center"
          >
            <img
              src={course.image}
              alt={course.title}
              className="w-full h-40 object-contain rounded-md mb-4 bg-gray-800 p-4"
            />
            <h3 className="text-lg font-semibold mb-2 text-center">{course.title}</h3>
            <p className="mb-1">
              <span className="font-medium">Duration: </span>
              {course.duration}
            </p>
            <p className="mb-4">
              <span className="font-medium">Cost: </span>
              {course.cost}
            </p>
            <div className="flex gap-3 mt-auto w-full">
              <Link
                to={`/courses/${course.id}`}
                className="flex-1 bg-blue-700 hover:bg-blue-600 text-white px-3 py-2 rounded text-center"
              >
                View Syllabus
              </Link>
              <a
                href={`https://wa.me/${whatsappNumber}?text=${sampleMessage}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex-1 bg-green-600 hover:bg-green-500 text-white px-3 py-2 rounded text-center"
              >
                Enroll Now
              </a>
            </div>
          </div>
        ))}
      </div>
    </div>
  </div>
);

export default Courses;
